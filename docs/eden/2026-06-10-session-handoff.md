...
- **Instant spoken acknowledgments:** `ackPhrases.ts` — 4 DE + 4 EN phrases ("Jawohl.", "Mache ich.", "Bin dran.", "Einen Moment." / EN equivalents) with sequential non-repeating rotation (`nextAckIndex`). `useAcks.ts` hook: prefetches TTS bytes for the current language once the session is ready; on submit, the cached audio chunk is enqueued immediately into `useSpeechQueue` before the agent turn starts (sub-100 ms to first sound when warm). Text fallback while the cache is cold or if prefetch failed. Speech queue (Round 2) was extended to accept pre-synthesized chunks (`SpeechChunk = string | { audio: ArrayBuffer }`) directly — no extra round-trip.
- **Live activity panel:** `activity.ts` data model — `ToolEntry {id, name, context, status, preview?, duration?, summary?}` per tool call, plus `TurnEntry {links[], images[]}` appended on `message.complete` with URLs extracted from the final text (`extractLinks`; image bucket by extension, query strings ignored); feed capped at 100 entries. `ActivityPanel.tsx` component: live tool event feed with context line, preview clip, duration badge, summary on completion, link chips (hostnames), image thumbnails. Panel is collapsible (chevron toggle), hidden below 980 px viewport width. Styles in `styles.css` (Arc Cyan palette, consistent with sphere/HUD).
